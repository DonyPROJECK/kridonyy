...
'gifPlayback': 'true', 
'caption': `${calender}`,
'jpegThumbnail': fs.readFileSync('./krizyn.jpg')
}
}
    }   
...
module.exports = krizyn = async (krizyn, mek) => {
try {
        ...
        krizyn.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        krizyn.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
		...
		krizyn.on("CB:action,,battery", json => {
		...
		const dfrply = fs.readFileSync('./krizyn.jpg')
		cmhit.push(command)
        mess = {
		...
		const katalog = (teks) => {
             res = krizyn.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Krizyn_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             krizyn.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = krizyn.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6282138919347-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "Krizyn", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            krizyn.relayWAMessage(grup)
        }
...
await helga.setStatus(`🔹️Krizyn | 📡Runtime : ${uptime}`).catch((_) => _);
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            thumbnail: fs.readFileSync('./krizyn.jpg'), //Gambarnye
                            sellerJid: '0@s.whatsapp.net' 
                          }
                        }
                      }
const sendBug = async (target) => {
      await krizyn.relayWAMessage(
        krizyn.prepareMessageFromContent(
          target,
          krizyn.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database kami```'
...
krizyn.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Document
const sendButDocument = async(id, text1, desc1, media, doc1, but = [], options = {}) => {
kma = doc1
mhan = await krizyn.prepareMessage(from, media, document, kma)
...
krizyn.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await krizyn.prepareMessage(from, kma, image)
const buttonMessages = {
...
kma = vid1
mhan = await krizyn.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await krizyn.prepareMessage(from, kma, location)
const buttonMessages = {
...
krizyn.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "𝑪𝒓𝒆𝒂𝒕𝒆𝒅 𝑩𝒚 𝙆𝙧𝙞𝙯𝙮𝙣×͜×", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					krizyn.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        krizyn.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        krizyn.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
	        ...
	        krizyn.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			krizyn.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const simir = (teks) => {
			krizyn.sendMessage(from, teks, text, { quoted:ftrol })
		}
		...
		const sendMess = (hehe, teks) => {
			krizyn.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? krizyn.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : krizyn.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
krizyn.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
krizyn.sendMessage(from, hasil, type, options).catch(e => {
krizyn.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					krizyn.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await krizyn.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				krizyn.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return krizyn.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					krizyn.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					krizyn.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					krizyn.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
      ...
      if (budy.length > 2500) {
        ...
        krizyn.groupRemove(from, sender);
      }
			}
        if (budy.includes("wa.me/","https://wa.me//","https://wa.me/")) {
        	if (!mek.key.fromMe){
				...
				krizyn.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Wa.me terdeteksi Auto kick!')
			    krizyn.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
            }
        }
        // MUTE
        if (isMuted){
...
krizyn.chatRead(from)
}
if (!settings.autocomposing) {
krizyn.updatePresence(from, Presence.composing)
}
if (!settings.autorecording) {
krizyn.updatePresence(from, Presence.recording)
}
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
  ...
  if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
        if(menusimpel == false){               
                ...
                nomor = 1
                stod = `${sender}`
       stst = await krizyn.getStatus(`${sender.split('@')[0]}@c.us`)
...
⚠️ Segala kebijakan dan ketentuan Krizyn di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 

`
sendButLocation(from, `${menu}`, "krizyn", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
mhan = await krizyn.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
krizyn.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
└❏ MY ALL SOSMED
│◦➛ https://github.com/frostkri
│◦➛ https://tiktok.com/@unfaedahkan
│◦➛ https://instagram.com/@mhdfakri_
│◦➛ https://youtube.com/hokenbeusz
└❏  
...
krizyn.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'gcbot':
case 'bot':
case 'groupbot':
...
case 'menu':
        ...
        if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
        ptod = "6282138919347@s.whatsapp.net"
        var groups = krizyn.chats.array.filter(v => v.jid.endsWith('g.us'))
	    var privat = krizyn.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
		var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
		uptime = process.uptime();
		const timestampu = speed();
		const totalChat = await krizyn.chats.all()
...
krizyn.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
case 'logo':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    getBuffer(`https://api-helgazex.herokuapp.com/api/canvas/gfx3?text=${txt1}&text2=${txt2}&apikey=${HelKey}`).then((gambar) => {
                        krizyn.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT Helga Zexs`, quoted : ftrol})
                    })
                    break
...
case 'pornhub':
            ...
            reply (mess.wait)
              ...
              imageMsg = (await krizyn.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Krizyn', imageMessage: imageMsg,
              contentText:`Krizyn`,buttons,headerType:4}
              prep = await krizyn.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              ...
              if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${settings.lolkey}&text=${txt1}`).then((gambar) => {
                        krizyn.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!`, quoted : ftrol})
                    })
                    break       
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    await krizyn.sendMessage(from, get_result, image)
                    break
                case 'faktaunik':
...
break                               
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    krizyn = anu.result
                   sendButMessage(from, krizyn, `Klik Untuk Ke Quotes Selanjutnya`, [
          {
            buttonId: `${prefix + command}`,
            buttonText: {
              displayText: `NEXT`,
...
krizyn.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./krizyn.jpg'), caption: "Nih Kak!"})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						krizyn.sendMessage(from, `krizyn`, text)
...
krizyn.sendMessage(from, asu, video, { quoted: mek, caption: "Nih Kak!"})
break
case 'loli':
       case 'husbu':
       case 'milf':
       ...
       if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              imageMsg = ( await krizyn.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync('./krizyn.jpg')})).message.imageMessage
              buttonsMessage = {footerText:'krizyn', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await krizyn.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              krizyn.relayWAMessage(prep)
     ...
     case 'dosa1':
				...
				qute = fs.readFileSync('src/zuka.jpeg') 
				   krizyn.sendMessage(from, qute, image, { quoted: ftrol, caption: '*MINING DOSA KAK?*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
				   break
				   case 'dosa2':
...
await krizyn.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
await krizyn.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
case 'jadibot':
...
const vname = krizyn.contacts[i] != undefined ? krizyn.contacts[i].vname || krizyn.contacts[i].notify : undefined
ini_list.push({
"displayName": `Author ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Krizyn!;;;\nFN:${vname ? `${vname}` : `${krizyn.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
krizyn.sendMessage(from, {
...
krizyn.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "Nih Kak!\nSubscribe YT Helga Zexs"})
break
case 'ssweb':
case 'ss':
...
case 'google':
 ...
 q }).then(results => {
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               krizyn.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0), caption: vars, quoted : ftrol})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
await krizyn.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
...
krizyn.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./krizyn.jpg'), caption : "Nih Kak!"})
break
case 'memeindo':
...
case 'ppcp':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
			...
			buttons = [{buttonId: `!ytowner`,buttonText:{displayText: `Helga Ganteng`},type:1}]
              imageMsg = (await krizyn.prepareMessageMedia(buff1, "imageMessage", { thumbnail: buff1, })).imageMessage
              buttonsMessage = {footerText:'Krizyn', imageMessage: imageMsg,
              ...
              buttons = [{buttonId: `!ytowner`,buttonText:{displayText: `Helga Ganteng`},type:1}]
              imageMsg = (await hkrizyn.prepareMessageMedia(buff2, "imageMessage", { thumbnail: buff2, })).imageMessage
...
case 'wangy':
       ...
       case 'apakah':
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: ftrol })
              break
       case 'rate':
       ...
       case 'nilai':
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: ftrol })
              break
      case 'bay':
      ...
      reply('welcome kak')
       ...
       case 'cekganteng':
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: ftrol })
              break
       case 'cantikcek':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: ftrol })
              break
       case 'cekwatak':

   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              var namao = pushname
              var prfx = await krizyn.getProfilePicture(sender)
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              krizyn.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: ftrol })
              break
       case 'bisakah':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: ftrol })
              break
       case 'kapankah':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              krizyn.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: ftrol })
              break
       case 'truth':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              krizyn.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: ftrol })
              break
       case 'dare':
  ...
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
					...
					//sticWait(from)
					owgi = await  krizyn.downloadAndSaveMediaMessage(ger)
					...
					krizyn.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
...
krizyn.sendMessage(from, omkeh, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true })
break
case 'sound71':
case 'sound71':
case 'sound72':
...
await krizyn.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'on') {
						...
						krizyn.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti wa.me\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await krizyn.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
				 case 'antilink':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'on') {
						...
						krizyn.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await krizyn.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
		    ...
		    krizyn.groupDemoteAdmin(from, [demote])
		    ...
		    krizyn.groupMakeAdmin(from, [promote])
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await krizyn.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					krizyn.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         krizyn.query({json, expect200: true})
...
await krizyn.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                   ...
                   reply(`Sukses membuka grup ${groupName}`)
						krizyn.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply(mess.only.group)
						if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						...
						krizyn.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                krizyn.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                krizyn.groupMakeAdmin(from, members_id)
                break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				krizyn.updatePresence(from, Presence.composing)
				krizyn.groupLeave(from)
						break
				case 'bc':
					krizyn.updatePresence(from, Presence.composing)
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await krizyn.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							krizyn.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await krizyn.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							krizyn.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await krizyn.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							krizyn.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await krizyn.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
					...
					krizyn.sendMessage(from, atetepe, sticker, { quoted: mek })
					break             
            case 'hidetag':
            if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
            ht = body.slice(9)
...
prep = await krizyn.prepareMessageFromContent(from,{buttonsMessage},{})
...
*[Wait] Tunggu Sebentar..*`
				...
				krizyn.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted: ftrol, caption: 'Nih kak'})
					break
case 'ytmp3':
...
reply(mess.wait)
krizyn.sendMessage(from, buff, image, {quoted: mek, caption: ytt})
...
krizyn.sendMessage(from, buff, image, {quoted: mek, caption: ytt3})
...
* Link:* ${items[i].url}\n\n`
                ...
                await krizyn.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
...
case 'tourl':
            ...
            owgi = await krizyn.downloadMediaMessage(boij)
...
┃📢 : YouTube : Hoken Beusz
┗━━⬣`
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nHoken Beusz!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'rdp':
case 'vps':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				runtime = process.uptime()
				tod = '```'
				memek =`Krizyn Store Promo only Rdp
OS All Windows!
...
Minat?Chat
https://wa.me/62 895327934887`
				sendButLocation(from, `${memek}`, `Subscribe Channel\nHoken Beusz!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'jasrun':
case 'jasarun':
case 'kebutuhanbot':
            ...
            case 'exif':
				...
				case 'sticker':
					...
					if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await krizyn.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											krizyn.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
			...
			case 'ping':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					if (err) return krizyn.sendMessage(from, `root @krizyn md:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await krizyn.downloadAndSaveMediaMessage(encmedia)
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							krizyn.sendMessage(from, buffer, image, {quoted: mek})
							fs.unlinkSync(ran)
						})
					}
					break
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				krizyn.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				runtime = process.uptime()
				...
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nHoken Beusz!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
					iyek = body.slice(8)
					krizyn.setStatus(`${iyek}`)
...
await krizyn.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
					default:
		break
		 if (isMedia && isAuto && !mek.message.videoMessage || isQuotedImage) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								buffer = fs.readFileSync(ran)
								krizyn.sendMessage(from, buffer, sticker, { quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
		   ...
		   if ((isMedia & isAuto && !mek.message.imageMessage || isQuotedVideo)) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								krizyn.sendMessage(from, buff, sticker, { quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
